...
import com.rocketmoon.draganddrop.databinding.ListItemRecyclerviewBinding
import java.util.Collections

interface ItemStartDragListener {
    // Drop이 됐을 때 Activity에서 사용할 메서드
    fun onDropActivity(initList : ArrayList<MainActivity.SampleData>, changeList: ArrayList<MainActivity.SampleData>)
}

class MainListAdapter : RecyclerView.Adapter<MainListAdapter.ViewHolder>(), ItemMoveListener {

    private var mSampleList: ArrayList<MainActivity.SampleData> = ArrayList()
    private var onItemDragListener: ItemStartDragListener? = null
    var initList: ArrayList<MainActivity.SampleData> = ArrayList()
    /**
     * Activity에서 호출할 메서드
     */
    ...
    inner class ViewHolder(private val mBinding: ListItemRecyclerviewBinding) : RecyclerView.ViewHolder(mBinding.root) {
        fun bind(mSampleData: MainActivity.SampleData){
            mBinding.tvTitle.text = mSampleData.title
        }
    }

    fun setData(sampleList: ArrayList<MainActivity.SampleData>){
